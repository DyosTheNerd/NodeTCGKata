...
describe("basic Hand interface", ()=>{
    ...
    it("should have a function to add a card to a players hand",()=>{
        expect(testedService.addCardToHand("playerA", "gameID",{cost:3})).to.be.equal(true)
    })

    describe("mechanics when adding cards", ()=>{
        it("should return a card just added when querying the hand",() =>{
            testCard = {cost:3}
            testedService.addCardToHand("playerA", "gameID",testCard)
        ...
        it("should return false, when the handsize becomes > 5 and not add the card to hand", ()=>{
            testedService.addCardToHand("playerA", "gameID",{cost:0})
            testedService.addCardToHand("playerA", "gameID",{cost:0})
            testedService.addCardToHand("playerA", "gameID",{cost:0})
            testedService.addCardToHand("playerA", "gameID",{cost:0})
            testedService.addCardToHand("playerA", "gameID",{cost:0})
            ...
            testCard = {cost:3}
            expect(testedService.addCardToHand("playerA", "gameID",testCard)).to.be.equal(false)
